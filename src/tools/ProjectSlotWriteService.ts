...
export class ProjectSlotWriteService {
  ...
  constructor(deps: WriteServiceDeps = {}) {
    ...
    this.slotService = deps.slotService ?? new SlotService();
  }
  ...
  private async updateSlot(slotId: number, input: SlotUpdateInput): Promise<ProjectSlotWriteResult> {
    ...
    if (normalized.disabled === false && slot.currentTicketId == null) {
      this.slotService.tryResumeQueue(slot.projectId ?? undefined).catch(console.error);
    }
    return {
      success: true,
